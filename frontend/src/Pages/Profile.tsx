    ...
    return (
        <div className="all_screen">
            <div className={`left ${responsive ? 'hidden md:flex' : 'flex w-full md:w-50 md:shrink-0'}`}>
                {/* //////////////////////////avatar + mon nom//////////////////////////////////// */}
                <div className="box_me" onClick={()=>{setSelectUser(Myself); setResponsive(true);}}>
                ...
                <div className="box_lst_friend">
                    {lstFriends.map((theFriend, idx) => (
                        <div className="display_friends" key={idx} onClick={()=>{setSelectUser(theFriend); setResponsive(true);}}>
            ...
            <div className={`box_profile ${responsive ? 'flex flex-1' : 'hidden md:flex md:flex-1'}`}>
                {responsive && (
                    <button className="md:hidden flex gap-2 bg-indigo-500 rounded-full items-center mr-auto p-2 " onClick={() => setResponsive(false)}> <IoIosArrowBack/></button>
                )}
                {isShowNotif === 1 ? (
